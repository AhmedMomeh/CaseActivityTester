...
namespace Shared.Activities
{
    ...
    /// purely by grade band.
    ///
    /// Rules:
    ///   IsSenior(gradeLevelText)  -> nextApprovalRoute = "CPCO"
    ///   else                      -> nextApprovalRoute = "HRDirectorOrAssociateDirector"
    ///
    /// Senior matching is case-insensitive substring-based (same matcher used
    /// by the other HR routing activities), so all of these count as senior
    /// and route to CPCO:
    ...
    /// </summary>
    public class HiringRequest_RouteByGradeActivity : ActivityTemplate
    {
        ...
        public override void Execute(WorkflowItem workflowItem)
        {
            ...
            LogInfo($"---- HiringRequest_RouteByGradeActivity BEGIN  DocumentId={documentIdStr} ----");
            ...
            try
            {
                ...
                LogInfo($"---- HiringRequest_RouteByGradeActivity nextApprovalRoute={nextApprovalRoute} ");
                LogInfo($"---- HiringRequest_RouteByGradeActivity END    DocumentId={documentId}  result=success ----");
            }
            catch (Exception ex)
            {
                LogException("Execute() failed", ex);
                LogInfo($"---- HiringRequest_RouteByGradeActivity END    DocumentId={documentIdStr}  result=FAILED ----");
            }
        }
        ...
        private static void Write(string level, string message)
        {
            try
            {
                string path = Path.Combine(LogDirectory,
                    "HiringRequest_RouteByGradeActivity-" + DateTime.Now.ToString("yyyy-MM-dd") + ".log");
